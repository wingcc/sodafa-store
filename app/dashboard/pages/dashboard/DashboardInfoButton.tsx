...
const DashboardInfoButton: React.FC<Props> = ({ title, description, bullets, hint }) => {
  ...
  const updatePosition = React.useCallback(() => {
    ...
    // Smart: if would go off bottom, try above
    if (top + ph > vh - margin) {
  ...
  return (
    <>
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className="w-6 h-6 rounded-full flex items-center justify-center border transition-colors bg-white dark:bg-white/5 border-gray-200 dark:border-white/10 text-gray-400 hover:text-[var(--color-darkGreen)] hover:border-[var(--color-darkGreen)]/20 hover:bg-[var(--color-darkGreen)]/5 dark:hover:text-white dark:hover:border-white/15 shrink-0"
      >
