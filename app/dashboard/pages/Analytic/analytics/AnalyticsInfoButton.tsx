  ...
  return (
    <div ref={ref} className="relative inline-flex">
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen(v => !v)}
        className="w-6 h-6 rounded-full flex items-center justify-center border transition-colors bg-white dark:bg-white/5 border-gray-200 dark:border-white/10 text-gray-400 hover:text-[var(--color-darkGreen)] hover:border-[var(--color-darkGreen)]/20 hover:bg-[var(--color-darkGreen)]/5 dark:hover:text-white"
      >
      ...
      {open && (
        <div
          role="dialog"
          aria-label={title}
          className={`absolute z-30 top-8 w-[300px] sm:w-[340px] rounded-2xl border shadow-xl backdrop-blur-xl bg-white dark:bg-[#131a28] border-gray-100 dark:border-white/10 p-4 ${isRTL ? 'left-0 sm:left-auto sm:right-0' : 'right-0'}`}
        >
